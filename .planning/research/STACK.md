...
**Domain:** Scheduled web-scraping + LLM-processing + API + human-review service (U.S. bankruptcy regulatory-change monitoring)
**Researched:** 2026-05-20
**Confidence:** HIGH (core framework, LLM SDK, datastore); MEDIUM (scheduler choice, scraping/PDF specifics — depends on observed source behavior)

## Recommended Stack

This is a **Python backend + Postgres + lightweight React review UI** stack. Python is the
recommended language because every hard part of this product — PDF/HTML parsing, the Anthropic
SDK, scheduling libraries, scraping tooling — has its strongest, best-documented ecosystem in
Python. A single-language backend keeps a small internal team productive.
...
The adaptive polling scheduler and the LLM pipeline are the operational heart of the product.
Recommendation: **Postgres-backed task queue, no separate broker.**
...
Rationale for Postgres-as-queue: at ~140 sources polled daily-to-hourly, throughput is tiny
(thousands of jobs/day, not millions). The operational cost of running and monitoring Redis or
RabbitMQ is not justified. Keeping queue state in the same Postgres instance also makes the
"detected vs effective date" timing logic transactional with the change records themselves.

### Web Scraping & Document Extraction

The source landscape is heterogeneous: RSS/notice feeds, static HTML, and PDF. Build a
**source-adapter abstraction** and pick the lightest tool per source.
...
Pattern: **normalize → hash-compare → difflib gate → LLM semantic diff.** Never send every poll
result to the LLM; cost and noise both explode. Store a normalized canonical text per rule so
diffs are stable against cosmetic markup changes.
...
Alternative for a faster v1: server-rendered HTML with **HTMX** + FastAPI templates. Viable if
the team has no front-end capacity — the review queue is mostly tables and forms. The React SPA
is recommended because the product is meant to become a sellable add-on and will likely grow a
richer UI; starting with a real front-end app avoids a rewrite.
...
**If v1 must ship as fast as possible (internal-only):**
...
**If/when productized into a multi-tenant add-on:**
...
**If a court source requires JS rendering:**
...
This is a **long-running scheduled service**, not a request/response-only app. Plan accordingly:
...
---
*Stack research for: U.S. bankruptcy regulatory-change monitoring service*
*Researched: 2026-05-20*
</content>
</invoke>
