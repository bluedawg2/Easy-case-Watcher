...
Plans:
- [ ] 01-01: Core domain models and schema — Source, Change (three date fields, status enum), Snapshot; Alembic migrations; nullable tenant_id on tenant-scoped tables
- [ ] 01-02: Source registry + RSS fetcher adapter with tri-state outcome and append-only snapshot store, seeded with one national feed
- [ ] 01-03: Hash-gate change detection and textual diff producing `detected` Change records
- [ ] 01-04: AI summary call (diff-grounded, not-legal-advice label) advancing Change to processed
- [ ] 01-05: Web review queue UI (approve/edit) and read-only pull delivery API for verified changes
**UI hint**: yes
